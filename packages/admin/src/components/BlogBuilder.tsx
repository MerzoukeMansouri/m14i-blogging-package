...
export function BlogBuilder({ sections, onChange, config: userConfig, components, onImproveContent, generatingSections, hideLayoutPicker }: BlogBuilderProps) {
  ...
  const [dragOverColumn, setDragOverColumn] = React.useState<{sectionIdx: number; columnIdx: number} | null>(null);
  ...
  const handleDragOver = (e: React.DragEvent) => {
    ...
    // Find closest column drop zone using data attributes
    const target = e.target as HTMLElement;
    const dropZone = target.closest('[data-drop-zone]') as HTMLElement;

    if (dropZone) {
      const sectionIdx = parseInt(dropZone.getAttribute('data-section-idx') || '-1');
      const columnIdx = parseInt(dropZone.getAttribute('data-column-idx') || '-1');

      if (sectionIdx >= 0 && columnIdx >= 0) {
        setDragOverColumn({ sectionIdx, columnIdx });
      }
    } else {
  ...
  const handleDragLeave = (e: React.DragEvent) => {
    // Only clear if leaving the entire builder
    const relatedTarget = e.relatedTarget as HTMLElement;
    if (!relatedTarget || !relatedTarget.closest('[data-editor-zone]')) {
      setDragOverColumn(null);
    }
  };
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    try {
      ...
      if (sectionIdx < 0 || columnIdx < 0) return;

      if (dragData.isLayout) {
        // Add layout section
        addLayout(dragData.type as LayoutType);
      } else {
        // Add block to specific column
        const newSections = [...sections];
        const section = newSections[sectionIdx];
        if (!section?.columns?.[columnIdx]) return;

        const newBlock = createDefaultBlock(dragData.type as ContentBlockType);
        section.columns[columnIdx].push(newBlock);
        onChange(newSections);

        // Auto-enter edit mode for the section
        setEditingSections(prev => {
          const next = new Set(prev);
          next.add(sectionIdx);
          return next;
  ...
  const handleBlockDragStart = (e: React.DragEvent, sectionIdx: number, columnIdx: number, blockIdx: number) => {
    const dragData = {
      type: "existing-block",
      sectionIdx,
      columnIdx,
  ...
  const handleBlockDrop = (e: React.DragEvent, destSectionIdx: number, destColumnIdx: number, destBlockIdx: number) => {
    e.preventDefault();
    e.stopPropagation();
    setDragOverColumn(null);
    ...
    try {
      ...
      if (dragData.type === "existing-block") {
        const { sectionIdx, columnIdx, blockIdx } = dragData;
        const newSections = [...sections];

        const sourceSection = newSections[sectionIdx];
        const destSection = newSections[destSectionIdx];

        if (!sourceSection?.columns?.[columnIdx] || !destSection?.columns?.[destColumnIdx]) return;

        // Remove from source
        const [movedBlock] = sourceSection.columns[columnIdx].splice(blockIdx, 1);

        // Insert at destination
        destSection.columns[destColumnIdx].splice(destBlockIdx, 0, movedBlock);

        onChange(newSections);
      }
    } catch (err) {
      console.error("Block drop failed:", err);
    }
  };
                ...
                {config.layouts.map((layout) => (
                  <div
                    ...
                    onDragStart={(e) => {
                      ...
                      const dragData = { type: layout.type, isLayout: true };
                      sessionStorage.setItem("dragData", JSON.stringify(dragData));
                    }}
              ...
              <div className="space-y-2">
                {config.blocks.map((block) => (
                  <div
                    ...
                    onDragStart={(e) => {
                      e.dataTransfer.effectAllowed = "copy";
                      const dragData = { type: block.type, isLayout: false };
                      sessionStorage.setItem("dragData", JSON.stringify(dragData));
                    }}
                    className="p-3 border rounded-lg cursor-grab flex items-center gap-3 hover:bg-accent active:cursor-grabbing"
                  >
                          ...
                          <div className={`grid gap-4 ${getLayoutClasses(section.type)}`}>
                            {(section.columns || []).map((column, columnIndex) => (
                              <div
                                ...
                                className={`min-h-[200px] border-2 border-dashed rounded-lg p-4 flex flex-col transition-all ${
                                  dragOverColumn?.sectionIdx === sectionIndex && dragOverColumn?.columnIdx === columnIndex
                                  ...
                                  <div className="space-y-3 w-full">
                                    {column.map((block, blockIndex) => (
                                      <div
                                        key={block.id}
                                        draggable
                                        onDragStart={(e) => handleBlockDragStart(e, sectionIndex, columnIndex, blockIndex)}
                                        onDragOver={(e) => {
                                          e.preventDefault();
                                          e.stopPropagation();
                                        }}
                                        onDrop={(e) => handleBlockDrop(e, sectionIndex, columnIndex, blockIndex)}
                                        className="group relative border rounded-lg bg-card hover:shadow-md transition-shadow cursor-move"
                                      >
                                        <div className="flex items-start gap-2 p-3">
                                          {/* Drag Handle */}
                                          <div className="cursor-grab active:cursor-grabbing opacity-50 hover:opacity-100 transition-opacity pt-1">
                                          ...
                                          <div className="flex-1 min-w-0">
                                            <ContentBlockInlineEditor
                                              ...
                                              onChange={(updatedBlock) =>
                                                updateBlock(sectionIndex, columnIndex, blockIndex, updatedBlock)
                                              }
                                              components={{
                                          ...
                                          <div>
                                            <button
                                              onClick={() => deleteBlock(sectionIndex, columnIndex, blockIndex)}
