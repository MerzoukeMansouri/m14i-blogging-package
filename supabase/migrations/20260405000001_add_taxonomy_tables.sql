...
CREATE INDEX idx_categories_display_order ON blog.categories(display_order);
CREATE INDEX idx_categories_created_by ON blog.categories(created_by);

CREATE INDEX idx_tags_slug ON blog.tags(slug);
CREATE INDEX idx_tags_usage_count ON blog.tags(usage_count DESC);
CREATE INDEX idx_tags_created_by ON blog.tags(created_by);
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION blog.get_categories_with_counts()
RETURNS TABLE (
  id UUID,
  name TEXT,
  slug TEXT,
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION blog.get_tags_with_counts()
RETURNS TABLE (
  id UUID,
  name TEXT,
  slug TEXT,
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION blog.ensure_category_exists(category_name TEXT)
RETURNS UUID AS $$
DECLARE
  category_id UUID;
  category_slug TEXT;
BEGIN
  ...
  IF category_id IS NULL THEN
    INSERT INTO blog.categories (name, slug)
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION blog.ensure_tag_exists(tag_name TEXT)
RETURNS UUID AS $$
DECLARE
  tag_id UUID;
  tag_slug TEXT;
...
GRANT SELECT ON blog.categories TO anon, authenticated;
GRANT SELECT ON blog.tags TO anon, authenticated;

-- Grant all on tables for authenticated users (RLS will handle actual permissions)
GRANT ALL ON blog.categories TO authenticated;
GRANT ALL ON blog.tags TO authenticated;

-- ============================================================================
-- COMMENTS & DOCUMENTATION
-- ============================================================================

COMMENT ON TABLE blog.categories IS 'Blog categories - allows dynamic category management per app';
COMMENT ON TABLE blog.tags IS 'Blog tags - flexible tagging system with usage tracking';

COMMENT ON FUNCTION blog.get_categories_with_counts() IS 'Get all categories with published post counts';
COMMENT ON FUNCTION blog.get_tags_with_counts() IS 'Get all tags with published post counts';
COMMENT ON FUNCTION blog.ensure_category_exists(TEXT) IS 'Helper to auto-create categories when assigning to posts';
COMMENT ON FUNCTION blog.ensure_tag_exists(TEXT) IS 'Helper to auto-create tags when assigning to posts';
...
-- Create categories
INSERT INTO blog.categories (name, slug, description, color, icon, display_order)
VALUES
...
-- Create tags
INSERT INTO blog.tags (name, slug, description, color)
VALUES
...
SELECT * FROM blog.get_categories_with_counts();

-- Get tags with counts
SELECT * FROM blog.get_tags_with_counts();

-- Auto-create category when creating post
SELECT blog.ensure_category_exists('New Category');
