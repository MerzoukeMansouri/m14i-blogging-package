...
-- Ensure auth.jwt() exists (not included in all Supabase image versions)
-- Owned by supabase_auth_admin so GoTrue can replace it during its own migrations
...
-- Public views so the Supabase JS client (which defaults to public schema)
-- can access blog tables as blog_posts / blog_media
CREATE OR REPLACE VIEW public.blog_posts AS SELECT * FROM blog.posts;
CREATE OR REPLACE VIEW public.blog_media AS SELECT * FROM blog.media;

GRANT SELECT ON public.blog_posts TO anon, authenticated;
GRANT SELECT ON public.blog_media TO anon, authenticated;
GRANT ALL ON public.blog_posts TO authenticated;
GRANT ALL ON public.blog_media TO authenticated;

-- Public schema wrappers for blog.* functions (Supabase JS client calls RPC in public by default)
CREATE OR REPLACE FUNCTION public.blog_search_posts(search_query TEXT)
RETURNS SETOF blog.posts AS $$
BEGIN
  RETURN QUERY SELECT * FROM blog.search_posts(search_query);
END;
$$ LANGUAGE plpgsql STABLE;

CREATE OR REPLACE FUNCTION public.blog_get_posts_by_tag(tag_name TEXT)
RETURNS SETOF blog.posts AS $$
BEGIN
  RETURN QUERY SELECT * FROM blog.get_posts_by_tag(tag_name);
END;
$$ LANGUAGE plpgsql STABLE;

CREATE OR REPLACE FUNCTION public.blog_get_posts_by_category(category_name TEXT)
RETURNS SETOF blog.posts AS $$
BEGIN
  RETURN QUERY SELECT * FROM blog.get_posts_by_category(category_name);
END;
$$ LANGUAGE plpgsql STABLE;

GRANT EXECUTE ON FUNCTION public.blog_search_posts TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.blog_get_posts_by_tag TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.blog_get_posts_by_category TO anon, authenticated;
